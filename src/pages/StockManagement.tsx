...
import { fetchStockItems, addStockItem, fetchBoxStock, addBoxStock, setupSupabaseTables, addMultiplePizzaStock } from '@/utils/supabase';
...
const StockManagement = () => {
  ...
  const [openMultiPizza, setOpenMultiPizza] = useState(false);
  ...
  // Submit form tambah multiple stok pizza
  const handleMultiPizzaSubmit = async (items: Omit<PizzaStock, 'id' | 'updatedAt'>[]) => {
    try {
      const addedStocks = await addMultiplePizzaStock(items);
      
      if (addedStocks && addedStocks.length > 0) {
        await loadStockData(); // Reload all stock data
        setOpenMultiPizza(false);
        
        toast({
          title: "Stok berhasil ditambahkan",
          description: `${addedStocks.length} jenis pizza berhasil ditambahkan ke stok.`,
        });
      } else {
        toast({
          title: "Terjadi kesalahan",
      ...
      >
        {activeTab === "pizza" ? (
          <div className="flex space-x-2">
            ...
            <Dialog open={openMultiPizza} onOpenChange={setOpenMultiPizza}>
              <DialogTrigger asChild>
              ...
              <MultiPizzaStockForm 
                onSubmit={handleMultiPizzaSubmit}
                onCancel={() => setOpenMultiPizza(false)}
              />
            </Dialog>
          </div>
        ) : (
        ...
        <Tabs defaultValue="pizza" className="mb-6" onValueChange={setActiveTab}>
          ...
          <TabsContent value="pizza" className="mt-6">
            <div className="grid grid-cols-1 gap-6">
              <StockAvailabilityTable stockItems={stockItems} />
